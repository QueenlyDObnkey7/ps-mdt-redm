...
RegisterNetEvent("RSGCore:Client:OnJobUpdate", function(data)
    print(data.name)
    if AllowedJob(data.name) then
        print("here")
    ...
    --    TriggerServerEvent('RSGCore:ToggleDuty')
        if PlayerData.job.name == "vallaw" or PlayerData.job.type == "leo" then
            TriggerServerEvent("police:server:UpdateCurrentCops")
        end
        if (PlayerData.job.name == "ambulance" or PlayerData.job.type == "ems") and data.name then
            TriggerServerEvent('hospital:server:AddDoctor', 'ambulance')
        elseif (PlayerData.job.name == "ambulance" or PlayerData.job.type == "ems") and not data.name then
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        RSGCore.Functions.Notify("Can't do that!", "error")
    end
end, false)
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    RSGCore.Functions.TriggerCallback("mdt:server:getWarrants", function(data)
        if data then
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Welcome, " ..PlayerData.job.grade.name..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2), location = "", fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    RSGCore.Functions.TriggerCallback('mdt:server:SearchProfile', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        RSGCore.Functions.TriggerCallback('mdt:server:GetProfileData', function(result)
            p:resolve(result)
        end, data)
    ...
    for i=1,#vehicles do
        ...
        local vehData = RSGCore.Shared.Vehicles[vehicle['vehicle']]
...
RegisterNUICallback("sendFine", function(data, cb)
    ...
    local p = promise.new()
    RSGCore.Functions.TriggerCallback('mdt:server:GetPlayerSourceId', function(result)
        p:resolve(result)
    end, citizenId)
    ...
    if not isLimited then
        ...
        for i=1, #vehicles do
            ...
            if tempModel and tempModel ~= "Unknown" then
                local vehData = RSGCore.Shared.Vehicles[tempModel]
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    RSGCore.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    RSGCore.Functions.TriggerCallback('mdt:server:SearchVehicles', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                RSGCore.Functions.Notify('Vehicle not found!', 'error')
...
RegisterNUICallback("searchWeapons", function(data, cb)
    ...
    RSGCore.Functions.TriggerCallback('mdt:server:SearchWeapons', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        RSGCore.Functions.Notify("You have set your radio frequency to "..radio..".", "success")
    else
        RSGCore.Functions.Notify("Invalid Station(Please enter a number)", "error")
    end
end)
...
function GetPlayerWeaponInfos(cb)
    RSGCore.Functions.TriggerCallback('getWeaponInfo', function(weaponInfos)
        cb(weaponInfos)
    end)
end
    ...
    GetPlayerWeaponInfos(function(weaponInfos)
        if weaponInfos and #weaponInfos > 0 then
            for _, weaponInfo in ipairs(weaponInfos) do
                ...
                TriggerEvent('RSGCore:Notify', "Weapon " .. weaponInfo.weaponmodel .. " has been added to police database.")
...
if Config.UseWolfknightRadar == true then
    ...
    AddEventHandler("ps-mdt:client:trafficStop", function()
        local plyData = RSGCore.Functions.GetPlayerData()
        local currentPos = GetEntityCoords(PlayerPedId())
        local locationInfo = getStreetandZone(currentPos)
        if not IsPedInAnyPoliceVehicle(PlayerPedId()) then
            RSGCore.Functions.Notify("Not in any Police Vehicle!", "error") 
            return 
        end
        ...
        if not coolDown then
            ...
            RSGCore.Functions.Notify("Traffic Stop Cooldown active!", "error") 
        end
    end)
end
