...
--------------------------------
--[[
RSGCore.Functions.CreateCallback('ps-mdt:server:MugShotWebhook', function(source, cb)
...
if Config.UseWolfknightRadar == true then
	...
	AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
		...
		if bolo == true then
			TriggerClientEvent('RSGCore:Notify', src, 'BOLO ID: '..boloId..' | Title: '..title..' | Registered Owner: '..vehicleOwner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end
		if warrant == true then
			TriggerClientEvent('RSGCore:Notify', src, 'WANTED - INCIDENT ID: '..incidentId..' | Registered Owner: '..owner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end

		if Config.PlateScanForDriversLicense and driversLicense == false and vehicleOwner then
			TriggerClientEvent('RSGCore:Notify', src, 'NO DRIVERS LICENCE | Registered Owner: '..vehicleOwner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
		end
...
RSGCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    TriggerClientEvent('RSGCore:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)
...
RegisterNetEvent("ps-mdt:server:ClockSystem", function()
    ...
    if PlayerData.job.onduty then
        ...
        TriggerClientEvent('RSGCore:Notify', source, "You're clocked-in", 'success')
		...
		TriggerClientEvent('RSGCore:Notify', source, "You're clocked-out", 'success')
		MySQL.query.await('UPDATE mdt_clocking SET clock_out_time = NOW(), total_time = TIMESTAMPDIFF(SECOND, clock_in_time, NOW()) WHERE user_id = @user_id ORDER BY id DESC LIMIT 1', {
			['@user_id'] = PlayerData.citizenid
		})
...
RSGCore.Functions.CreateCallback('mdt:server:SearchProfile', function(source, cb, sentData)
...
RSGCore.Functions.CreateCallback('ps-mdt:getDispatchCalls', function(source, cb)
    local calls = exports['ps-dispatch']:GetDispatchCalls()
    cb(calls)
end)

RSGCore.Functions.CreateCallback("mdt:server:getWarrants", function(source, cb)
    local WarrantData = {}
...
RSGCore.Functions.CreateCallback('mdt:server:OpenDashboard', function(source, cb)
	local PlayerData = GetPlayerData(source)
...
RSGCore.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
	...
	if Config.UsingPsHousing and not Config.UsingDefaultQBApartments then
		...
		if propertyData and next(propertyData) then
			...
			if #apartmentList > 0 then
				...
				TriggerClientEvent("RSGCore:Notify", src, 'The citizen does not have an apartment.', 'error')
				print('The citizen does not have an apartment. Set Config.UsingPsHousing to false.')
			end
		else
			TriggerClientEvent("RSGCore:Notify", src, 'The citizen does not have a property.', 'error')
	...
	if id then
		...
		if Config.RemoveWeaponsPerms[Player.PlayerData.job.name] then
			if Config.RemoveWeaponsPerms[Player.PlayerData.job.name][Player.PlayerData.job.grade.level] then
				...
				local fullname = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
				TriggerClientEvent("RSGCore:Notify", src, 'No Permissions to do that!', 'error')
...
RSGCore.Functions.CreateCallback('mdt:server:SearchVehicles', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local src = source
	local PlayerData = GetPlayerData(src)
...
RSGCore.Functions.CreateCallback('mdt:server:SearchWeapons', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local PlayerData = GetPlayerData(source)
	if not PermCheck(source, PlayerData) then return cb({}) end
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	local src = source
	local targetPlayer = RSGCore.Functions.GetPlayerByCitizenId(cid)
	...
	if radio ~= nil then
		...
		TriggerClientEvent("RSGCore:Notify", src, targetName..' does not have a radio!', 'error')
	end
end)
...
RSGCore.Functions.CreateCallback('mdt:server:GetPlayerSourceId', function(source, cb, targetCitizenId)
    local targetPlayer = RSGCore.Functions.GetPlayerByCitizenId(targetCitizenId)
    if targetPlayer == nil then 
        TriggerClientEvent('RSGCore:Notify', source, "Citizen seems Asleep / Missing", "error")
        return
    end
    local targetSource = targetPlayer.PlayerData.source
...
RSGCore.Functions.CreateCallback('getWeaponInfo', function(source, cb)
    local Player = RSGCore.Functions.GetPlayer(source)
    local weaponInfos = {}
			...
			if string.find(item.name, "WEAPON_") then
				...
				if invImage then
					weaponInfo = {
						...
						weaponmodel = RSGCore.Shared.Items[string.lower(item.name)].label,
						weaponurl = invImage,
			...
			if item.type == "weapon" then
				...
				if invImage then
					local weaponInfo = {
						...
						owner = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
						weaponmodel = RSGCore.Shared.Items[item.name].label,
						weaponurl = invImage,
						notes = "Self Registered",
						weapClass = "Class 1",
					}
...
local function giveCitationItem(src, citizenId, fine, incidentId)
	...
	TriggerClientEvent('RSGCore:Notify', src, PlayerName.." (" ..citizenId.. ") received a citation!")
	if Config.QBBankingUse then 
		exports['rsg-banking']:AddMoney(Officer.PlayerData.job.name, fine) 
	end
	TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, RSGCore.Shared.Items['mdtcitation'], "add")
...
RegisterNetEvent('mdt:server:removeMoney', function(citizenId, fine, incidentId)
	...
	if not antiSpam then
		if Player.Functions.RemoveMoney('bank', fine, 'lspd-fine') then
			TriggerClientEvent('RSGCore:Notify', Player.PlayerData.source, fine.."$ was removed from your bank!")
			giveCitationItem(src, citizenId, fine, incidentId)
		else
			TriggerClientEvent('RSGCore:Notify', Player.PlayerData.source, "Something went wrong!")
		end
		antiSpam = true
		...
		TriggerClientEvent('RSGCore:Notify', src, "On cooldown!")
	end
end)
...
if Config.InventoryForWeaponsImages == "ox_inventory" and Config.RegisterWeaponsAutomatically then
	exports.ox_inventory:registerHook('buyItem', function(payload)
		...
		CreateThread(function()
			local owner = RSGCore.Functions.GetPlayer(payload.source).PlayerData.citizenid
	...
	if Config.RegisterCreatedWeapons then
		exports.ox_inventory:registerHook('createItem', function(payload)
			...
			CreateThread(function()
				local owner = RSGCore.Functions.GetPlayer(payload.inventoryId).PlayerData.citizenid
				if not owner or not payload.metadata.serial then return end
...
function GetPlayerDataById(id)
    local Player = RSGCore.Functions.GetPlayerByCitizenId(id)
    if Player ~= nil then
...
function GetPlayerLicenses(identifier)
    ...
    local Player = RSGCore.Functions.GetPlayerByCitizenId(identifier)
    if Player ~= nil then
