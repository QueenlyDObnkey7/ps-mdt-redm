...
// Parse `charinfo` if it's a string
...
// Ensure `metadata.licences` exists
if (!metadata.licences) {
  metadata.licences = {};
}

// Generate name from charinfo
let name = `${charinfo.firstname} ${charinfo.lastname}`;
let warrant = "red-tag";
let convictions = "red-tag";

// Handle licences display
let licences = "";

// Define fallback for license types
const defaultLicenseTypes = ["driver", "weapon", "business"]; // Add your expected licenses here

// Get licences from metadata, or default to fallback
let licArr = Object.entries(metadata.licences || {});

// If no licences are found, default to predefined license types with all set to false
if (licArr.length === 0) {
  licArr = Object.entries(
    defaultLicenseTypes.reduce((licenseObj, licenseType) => {
      licenseObj[licenseType] = false; // Default all license types to false
      return licenseObj;
    }, {})
  );
}

// Generate licence tags if player has access
